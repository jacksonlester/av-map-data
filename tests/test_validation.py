...
def test_csv_headers(csv_file):
    ...
    expected_headers = [
        ...
        'vehicles', 'platform', 'fares', 'direct_booking', 'flexibility', 'supervision',
...
def test_event_types(csv_file):
    ...
    valid_event_types = [
        ...
        'access_policy_changed', 'flexibility_updated', 'platform_updated', 'fleet_partner_changed'
    ]
    errors = []
        ...
        for row_num, row in enumerate(reader, start=2):
            if row.get('event_type') == 'service_created':
                required_fields = ['company', 'city', 'vehicles', 'platform', 'fares',
                                 'direct_booking', 'flexibility', 'supervision', 'access']
...
def test_service_attribute_values(csv_file):
    ...
    with open(csv_file, 'r') as f:
        ...
        for row_num, row in enumerate(reader, start=2):
            ...
            valid_flexibility = ['Point-to-Point', 'Stop-to-Stop']
            if flexibility and flexibility not in valid_flexibility:
                errors.append(f"Row {row_num}: flexibility must be one of {valid_flexibility}, got: {flexibility}")
